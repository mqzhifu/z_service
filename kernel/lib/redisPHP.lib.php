...
//	public $port = '6379';
    ...
    static function getServerConnFD($config_key = null){
        ...
        self::$conn = new Redis();
        self::$conn->pconnect($config['host'],$config['port'] ,20 );
    ...
    static function getServerPoolConnFD($config_key = null){
        if(!$config_key){
            $config = $GLOBALS['redis'][DEF_REDIS_CONN];
...
//    }
    ...
    //     * 增，构建一个列表(先进后去，类似栈)
    static public function lpush($key,$value,$isJson = 0 ){
        if($isJson){
            $value = json_encode($value);
        }

        return self::getServerConnFD()->lPush($key,$value);
    }
     ...
     */
    static function rPop($key,$isJson = 0){
        $rs = self::getServerConnFD()->rPop($key);
        if(!$rs){
            return 0;
        }
    ...
    static function getLockByKey($key,$expire){
