...
class DbLib{
    //配置文件数组结构
    public $_configArr = array('type','timeout','persistence','masterSlave','master');
    public $_configSubArr = array('host','port','user','pwd','db_name','key');
...
//    public $mTransaction = 0;//是否为事务，如果是则出现错误后是要抛出异常的
    ...
    //=================================
    static function getDbFD($config){
        if( arrKeyIssetAndExist(self::$_connPool,$config['key']) ){
            return array('m'=>self::$_connPool[$config['key']]['masterFD'],'s'=>self::$_connPool[$config['key']]['slaveFD']);
        }

        $masterHost = $config['master']['host'].($config['master']['port']?":{$config['master']['port']}":'');
        //是否 长连接
        if($config['persistence']){
            $masterHost = "p:".$masterHost;
        }
        $masterFD = mysqli_connect( $masterHost, $config['master']['user'], $config['master']['pwd'],$config['master']['db_name']);
        if ( !$masterFD ){
            ExceptionFrameLib::throwCatch("connect db error:". mysqli_error($masterFD) . " mysql_connect [connect db error]");
        }

        //设置字符集
        self::setNames($masterFD , $config['master']['char']);

        if($config['masterSlave']){//是否开启主从模式
            $slaveHost = $config['slave']['host'].($config['slave']['port']?":{$config['slave']['port']}":'');
            if($config['persistence']){
                $slaveHost = "p:".$slaveHost;
            }

            $slaveFD = mysqli_connect( "p:". $slaveHost, $config['slave']['user'], $config['slave']['pwd'],$config['master']['db_name']);
            self::setNames($slaveFD , $config['slave']['char']);
        }else{
            $slaveFD = $masterFD;
        }

        self::$_poll[$config['key']]['masterFD'] = $masterFD;
        self::$_poll[$config['key']]['slaveFD'] = $slaveFD;
        self::$_poll[$config['key']]['connTime'] = time();

        return array('m'=>$masterFD,'s'=>$slaveFD);
    }

    static function setNames($fd,$char){
    ...
    public function query($sql) {
        ...
        $this->_mysqlMemPoint = mysqli_query($this->mSlaveFD,$sql);
...
//            }
    ...
    public function execute($sql) {
        ...
        // 记录开始执行时间
        G('queryStartTime');
        $result =   mysqli_query( $this->mMasterFD,$sql) ;
        $this->debug($sql);
        if ( false === $result) {
            $err = $this->error($this->mMasterFD);
...
//        LogLib::appWriteFileHash($sql);
    ...
    public function escapeString($str) {
        ...
        return mysqli_real_escape_string($this->mMasterFD,$str);
    }
    //debug
    ...
    function authConfig($db_key){
        ...
        $config =  $GLOBALS['db_config'][$db_key];
        if(!$config){
        ...
        $config['key'] = $db_key;
    	foreach($this->_configArr as $k=>$v){
    		if( !$config[$v] ) ExceptionFrameLib::throwCatch('db_config para error','DB');
    	}

        foreach($this->_configSubArr as $k=>$v){
            if( !$config['master'][$v] ) ExceptionFrameLib::throwCatch('db_sub_config para error','DB');
        }
