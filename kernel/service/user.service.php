...
class UserService{
    ...
    //注册
    //type:类型
    function register($name,$ps,$type = 0,$clientInfo = null,$pcInfo = null){
        if(!$name){
            return out_pc(8009, "未填写账号");
        }
        ...
        if(!arrKeyIssetAndExist($data,'nickname')){
            $data['nickname'] = "";
        }
...
//        $data['client_info'] =json_encode($clientInfo);
        ...
        if(APP_NAME == IS_NAME || APP_NAME == 'open'){
...
//            LogLib::appWriteFileHash($sign);
    ...
    function loginCellphonePS($cellphone,$ps,$clientInfo){
    ...
    function pcLoginCellphonePs($cellphone,$ps){
        $rs = $this->selfLogin($cellphone,$ps,UserModel::$_type_pc_cellphone_ps);
        return $rs;
    }

    function pcLoginCellphoneSMS($cellphone,$smsCode){
        $rs = $this->selfLogin($cellphone,null,UserModel::$_type_pc_cellphone_sms,$smsCode);
        return $rs;
    }
    ...
    //3方登陆
    function thirdLogin($thirdUid,$type){
        $rs = $this->getThirdUser($thirdUid,$type);
        if($rs){
...
//        LogLib::appWriteFileHash(['para:',$name,$ps,$type]);
        ...
        if($type == UserModel::$_type_cellphone || $type == UserModel::$_type_pc_cellphone_sms){
...
//        LogLib::appWriteFileHash($verifySig);
...
//        LogLib::appWriteFileHash(["upUserDetailInfo%%%%%%%%%%%%%%%%%%",$info]);
        $data = array();
...
//            }
    ...
    function getTypeMethod($type ){
        $self = array(UserModel::$_type_cellphone,UserModel::$_type_name,UserModel::$_type_email,UserModel::$_type_cellphone_ps);
        if(in_array($type,$self)){
            return "self";
        }elseif($type == UserModel::$_type_guest){
            return "guest";
        }else{
            return "third";
        }
